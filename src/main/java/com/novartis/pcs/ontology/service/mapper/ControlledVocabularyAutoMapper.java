...
public class ControlledVocabularyAutoMapper
    ...
    public void execute(boolean sendNotification) {
        ...
        try {
            ...
            if (!vocabTerms.isEmpty()) {
                ...
                List<ControlledVocabularyTerm> unmapped = new ArrayList<>(vocabTerms.size());
                    ...
                    if (!vocabTerm.isExcluded()) {
                        ...
                        if (matchedTerm != null) {
                            ...
                            if (!previouslyRejected) {
                                ...
                                String msg = "Auto-mapping controlled vocab term \"" +
                                        vocabTerm.getName() +
                                        "\" from " +
                                        vocabTerm.getControlledVocabulary().getDatasource().getAcronym() +
                                        " controlled vocabulary " +
                                        vocabTerm.getControlledVocabulary() +
                                        " to term \"" +
                                        matchedTerm.getName() +
                                        "\" from ontology " +
                                        matchedTerm.getOntology().getName();
                                logger.info(msg);
    ...
    private void sendUnmappedEmail(List<ControlledVocabularyTerm> unmapped) {
        try {
            ...
            unmapped.sort(new ControlledVocabularyTermSourceComparator());
