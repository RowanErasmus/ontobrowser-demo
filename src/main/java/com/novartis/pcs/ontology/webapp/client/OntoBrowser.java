...
public class OntoBrowser implements EntryPoint, ValueChangeHandler<String> {
    ...
    private final Collection<MenuItem> ontologyMenuItems = new ArrayList<>();
    ...
    public void onModuleLoad() {
        ...
        service.loadCurrentCurator(new AsyncCallback<Curator>() {
            @Override
            public void onSuccess(Curator loggedInCurator) {
                curator = loggedInCurator;
                createPopups(loggedInCurator);
    ...
    private void createMenus(List<Term> terms) {
        ...
        for (final Term term : terms) {
            ...
            menu.addItem(ontology.getName(), () -> History.newItem(term.getReferenceId()));
    ...
    private void setUsername() {
        if (curator != null) {
            String username = curator.getUsername() != null ? curator.getUsername() : "Unknown user";
            createUsernameOrLoginItem(username);
        } else {
            service.getUserName(new AsyncCallback<String>() {
                ...
                public void onSuccess(String string) {
                    createUsernameOrLoginItem(string);
                }
            });
        }
    }

    private void createUsernameOrLoginItem(String username) {
        final String id = "login_button";
        // button that displays name and does nothing, login logic has been deleted for this public ontobrowser copy
        MenuItem login = new MenuItem(SafeHtmlUtils.fromString("Welcome " + username));
        login.getElement().setId(id);
    ...
    private MenuItem createPopupMenuItem(MenuBar menu, final String text, final OntoBrowserPopup popup) {
        return menu.addItem(text, popup::show);
    }
}
