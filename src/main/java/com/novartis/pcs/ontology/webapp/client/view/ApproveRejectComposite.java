...
public abstract class ApproveRejectComposite<T extends VersionedEntity> extends Composite
    ...
    private final ProvidesKey<T> keyProvider = new EntityKeyProvider<>();
    private final VersionedEntityListDataProvider<T> dataProvider = new VersionedEntityListDataProvider<>(keyProvider);
    private final CellTable<T> table = new CellTable<>(keyProvider);
    ...
    public ApproveRejectComposite(Entity entity,
        ...
        if (curator != null && curator.isAuthorised(entity)) {
            selection = new MultiSelectionModel<>(keyProvider);
                    ...
                    ApproveRejectComposite.this.service.approve(selected,
                            ...
                            new AsyncCallback<Set<T>>() {
                                ...
                                public void onSuccess(Set<T> approved) {
                                    processing.removeAll(selected);
                                    for (T entity1 : selected) {
                                        selection.setSelected(entity1, false);
                                    }
                    ...
                    ApproveRejectComposite.this.service.reject(selected,
                            ...
                            new AsyncCallback<Set<T>>() {
                                ...
                                public void onSuccess(Set<T> rejected) {
                                    processing.removeAll(selected);
                                    for (T entity12 : selected) {
                                        selection.setSelected(entity12, false);
                                    }
                                    dataProvider.getList().removeAll(selected);
    ...
    protected void setupTable() {
	     ...
	     * functionality with shift key is better anyway and have included
		...
		*/
        ...
        if (curator != null) {
            IconActionCell.Delegate<T> delegate = entity -> {
                editView.setEntity(entity);
                editView.show();
            };

            Column<T, T> editColumn =
                    new Column<T, T>(new IconActionCell<>(ImageResources.INSTANCE.editIcon(), delegate)) {
                        @Override
        ...
        if (curator != null) {
            ...
            Column<T, T> deleteColumn =
                    new Column<T, T>(new IconActionCell<>(ImageResources.INSTANCE.deleteIcon(),
                            delegate)) {
                        @Override
                        public T getValue(T entity) {
        ...
        ListHandler<T> sortHandler = new ListHandler<>(dataProvider.getList());
